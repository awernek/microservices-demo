...
public class Worker : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        logger.LogInformation("[SmsService] Topologia declarada.");
        ...
        logger.LogInformation("[SmsService] Pronto. Aguardando notificações de SMS...");
    ...
    private async Task WaitForRabbitMqAsync(ILogger logger, CancellationToken ct)
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            try
            {
                ...
                logger.LogInformation("[SmsService] Conectado ao RabbitMQ!");
                return;
            }
            catch
            {
                logger.LogWarning("[SmsService] RabbitMQ não está pronto. Tentando novamente em 3s...");
